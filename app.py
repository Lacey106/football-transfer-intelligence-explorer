...
def resolve_data_dir() -> Path:
    essential_files = [
        ...
        REQUIRED_FILES["clubs"],
        REQUIRED_FILES["transfers"],
    ]
    ...
    for candidate in candidates:
        if all((candidate / filename).exists() for filename in essential_files):
            return candidate

    checked = "\n".join(str(path) for path in candidates)
    required = ", ".join(essential_files)
    raise FileNotFoundError(
        f"Could not locate the processed data files.\n"
